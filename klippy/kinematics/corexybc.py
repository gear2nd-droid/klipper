...
class CoreXYBCKinematics:
    ...
    def set_position(self, newpos, homing_axes):
        logging.info('homing_axes:{0}'.format(homing_axes))
        for i in range(5):
            self.rails[i].set_position(newpos)
            if "xyzbc"[i] in homing_axes:
                self.limits[i] = self.rails[i].get_range()
    def clear_homing_state(self, clear_axes):
        for axis, axis_name in enumerate("xyzbc"):
    ...
    def check_move(self, move):
        ...
        if not move.axes_d[2]:
            return
        if(move.axes_d[2]):
            buf_y = self.bed_diameter / 2.0
            buf_x = self.bed_diameter / 2.0 * math.cos(-bpos / 180.0 * math.pi)
            # f(x,y) = x^2 / buf_x^2 + y^2 / buf_y^2 - 1.0 > 0.0
            fxy = xpos**2 / buf_x**2 + ypos**2 / buf_y**2 - 1.0
            if fxy < 0.0:
                bed_level = xpos * math.sin(-bpos / 180.0 * math.pi)
            else:
                bed_level = limits[2][0]
            if(zpos < bed_level):
